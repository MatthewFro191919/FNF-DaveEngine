...
local modRemap = {}
local percentFriends = {'invert', 'flip'}
...
modRemap.tiny = function(percent)
	return 'mini', percent / 100
end
for _,v in pairs{'x', 'y', 'z'} do
	modRemap['tiny'..v] = function(percent)
		return 'mini'..v, percent / 100
	end
end
...
local function queueFunc(mods, func, players)
...
function reset(params)
	...
	for _,player in pairs(players) do
		for mod,percent in pairs(default_mods) do
			if not params.exclude or not table.contains(params.exclude, mod) then
				set {params[1], mod, percent, plr = player}
			end
		end
	end
end
...
local function reverseCenterOffset(i, offset)
	local t = {-offset * .5, -offset * 1.5, offset * 1.5, offset * .5}
	return t[i + 1]
end